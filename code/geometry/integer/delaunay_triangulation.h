...
struct Edge {
    pt origin; Edge* rot = nullptr, *onext = nullptr; bool used = false;
...
bool left_of(pt p, Edge* e) { return gt(p.cross(e->origin, e->dest()), 0); }
bool right_of(pt p, Edge* e) { return lt(p.cross(e->origin, e->dest()), 0); }
    ...
    while (true) {
        ...
        if (valid(lcand)) {
            while (in_circle(basel->dest(), basel->origin, lcand->dest(), lcand->onext->dest())) {
                Edge* t = lcand->onext; delete_edge(lcand); lcand = t;
            }
        }
